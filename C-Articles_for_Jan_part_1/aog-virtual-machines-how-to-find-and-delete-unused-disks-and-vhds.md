...
通过检查托管磁盘的所有者来判断该磁盘是否正在使用。具体实现请下载[相关的 Power Shell 脚本](https://github.com/wacn/AOG-CodeSample/blob/master/VirtualMachines/PowerShell/aog-virtual-machines-how-to-find-and-delete-unused-disks-and-vhds/GetAndRemoveDisks.ps1)，并执行以下命令：
...
非托管磁盘源 .vhd 文件以页 Blob 的方式存储在存储账户中，通过查看页 Blob 的名称来判断是否是源 .vhd 文件，通过检查属性 LeaseState 来判断源文件是否被租用，然后进行删除。具体实现脚本，请[点击查看](https://github.com/wacn/AOG-CodeSample/blob/master/VirtualMachines/PowerShell/aog-virtual-machines-how-to-find-and-delete-unused-disks-and-vhds/GetAndRemoveVhds.ps1)。
